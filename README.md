...
需求文本 → 0·多轮澄清（LLM 保守判断需求缺失，至多追问一问，选项卡片交互）
        → A·世界知识提案（LLM 自由生成多日行程：主选 + note + alternates + 招牌/片区/餐窗规则，prompt v7）
...
- **餐窗保障**：正餐 POI 优先占窗、到达距饭点 ≤30min 先吃再逛、游完就地补餐、窗尾宽限与收尾晚餐提前量
...
PY=python3 bash scripts/release_gate.sh          # 发布门禁：编译→单测→回归→8 城库校验（push/PR 由 GitHub Actions 自动执行）
...
python scripts/test_food_window_priority.py      # 餐窗优先权
python scripts/test_faithful_mode.py             # 忠实执行模式
python scripts/test_capacity_guard.py            # 挪点容量预检
...
scripts/      33 个运维与测试脚本 4,068 行（扩城/扩库/校验/门禁/探针/回归/守卫/缓存补缺）
...
- **M14** 正确性审计与守卫：跨零点闭店归一化（救活两个「死点」）、KTV 新子类（4 家广州 KTV）、城市注册表守卫、缓存改动纯度校验

## Roadmap

- **P0** 行程交互式微调（点选换/删/换时段，复用 reuse 增量重解）· 城市库批量扩充（深圳/西安/厦门/青岛/长沙）· 预算感知（price 进决策 + 费用明细）
- **P1** 召回层补强（让「能排但没被提案」的远郊/主题点可落地）· 偏好记忆 · 评测集扩至 30+ 真实查询进发布门禁 · 文案 LLM 缓存 · 热门时段避让 · 观测看板
- **P2** 跨城交通衔接（高铁段作为日间转移）· 多人协作投票 · DeepSeek 流式输出 · 国际化（en_name）

### 已知未修缺陷

- `toptw.py` depot 换位失效（Python 元组赋值陷阱 →「无酒店时 depot 取主选第一点」从未生效；当前影响≈0，修它须同时把换位移到 `RoutingIndexManager` 构造之前）
- `family_ok=false` 仅在离线链路生效（主链路不读该字段；实测 LLM 自觉避开，是概率不是保证）
- TOPTW 不惩罚行程内部空档（evening 软窗 + 点少的天会拉出数小时空窗，通用场景未治）
