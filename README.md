...
- **提案层**（prompt v10：**常开硬规则 + 条件规则按需注入**）：7 条常开硬规则（全天大点独占日、远郊大点片区、跨天片区分散、配套顺路、正餐、备选、远郊深度体验）始终注入；6 条条件规则仅在判据命中时注入（招牌体验/亲子/慢节奏/贯穿偏好/住宿锚点/点名）。v9 把 14 条规则全量堆进 prompt 且靠散文声明优先级（贯穿偏好覆盖片区分散、慢节奏豁免傍晚密度），v10 让冲突从根源消失——慢节奏命中时傍晚密度规则根本不进 prompt。判据复用 `sequencer` 现成正则，普通查询 prompt 2849→1567 字符
...
- **12 城 722 POI**（广州 79 / 成都 75 / 苏州 75 / 北京 74 / 上海 69 / 杭州 57 / 武汉 50 / 南京 49 / 西安 54 / 重庆 48 / 长沙 47 / 盐城 45），46,575 对真实路网交通缓存（L1 OSRM + L2 高德实况，L3 直线兜底），库内点位实拍图 **722/722 = 100% 覆盖**
- **硬约束体系**：营业时间、闭馆日（日期感知，全库回填）、**跨零点闭店归一化**、餐窗美食（每窗 ≤1，咖啡馆非正餐分流）、返程酒店锚点、主题每日里程预算（骑行/徒步/自驾）
...
- **跨城联游**：「苏杭 4 天」自动识别跨城，逐城规划合并 + 住宿城市天数倾斜；**每城天数尊重用户明说的诉求**（「苏州玩2天」按确定性正则解析，不再 divmod 盲分）
- **城际转移段**（`src/intercity.py`）：跨城驾车时长走三级模型（磁盘缓存 → 高德 driving → 直线×1.25÷75km/h 兜底，**不能复用 `travel_hours`**——它的市内速度模型会把苏州→杭州算成 9.6h）。≥4h 独占转移日（>10h 按单日上限拆多天）、<4h 顺延抵达日起始时刻、「晚上9点开车去苏州」识别为夜间转移挂在出发城当天末尾；转移段在时间轴与地图上均可见
- **自驾主题**：`自驾/租车/驾车` 命中 driving 画像（每日 400km 预算，作用于含城际段的天），hop 标签显示「自驾」
...
python scripts/eval_regression.py [--llm]        # 回归评测：18 固化用例 / 12 城，离线 CI / --llm 真实全链路
python scripts/probe_perf.py                     # 性能探针：LLM/TOPTW 各环节耗时插桩
python scripts/release_gate.py                   # 发布门禁：编译→单测→回归→12 城库校验（纯 Python，本机与 CI 同一条命令）
python scripts/validate_city.py                  # POI 库全量校验

# 单个专项单测（改对应模块时按需跑）
python scripts/test_city_registry.py             # 城市注册表守卫（12 城×7 表，开城/加城必跑）
...
| `/api/route` | GET | 高德路径规划代理：`mode=riding\|walking\|driving&o=lng,lat&d=lng,lat`（磁盘缓存；`driving` 用于城际转移腿） |
| `/api/photo` | GET | POI 实拍图（缓存 746 条，12 城 100%，缺失回退瓦片） |
| `/api/share` | POST / GET | 行程快照保存 / 读取（`GET /api/share/<id>`） |
| `/api/stats` | GET | 调用量/延迟/补位命中率统计 |

> ⚠️ 行程响应中 `result.itinerary.days[].timeline[]` 的元素字段为 `type`（`poi`/`hop`/`meal`/`hotel`/`transfer`）/ `id` / `name` / `start` / `end`，**不是** `kind` / `poi_id`；`city_meta` 不下发 open/close/rating。多城联游时 `hotel` 只在住宿城那几天出现（出发/返回行），路线绘制以「该天时间轴是否含 `hotel` 行」为判据。
>
> `transfer`（城际转移段）额外带 `min` / `km` / `from` / `to` / `source` / `leg` / `n_legs`，`night=true` 表示夜间转移（挂在出发城当天末尾）。整天为转移日时 `days[].transfer_day=true`；抵达日被车程顺延时 `days[].arrive_transfer=true`。
...
- **P0** 行程交互式微调（点选换/删/换时段，复用 reuse 增量重解）· 城市库批量扩充（**已开盐城（第 9 城）+ 西安/重庆/长沙（第 10–12 城），全库 12 城 722 点；下一城候选深圳/厦门/青岛/天津/昆明**）· 预算感知（price 进决策 + 费用明细）
- **P1** ~~召回层补强~~ ✅ 已完成（M15）· 偏好记忆 · 评测集扩至 30+ 真实查询进发布门禁 · 文案 LLM 缓存 · 热门时段避让 · 观测看板
- **P2** 跨城交通衔接（**自驾段已实现**：`src/intercity.py` 真实驾车时长 + 转移日 / 夜间转移 / 抵达日顺延；**高铁段待接入**，>12h 车程目前只提示改乘）· 多人协作投票 · DeepSeek 流式输出 · 国际化（en_name）
...
- TOPTW 不惩罚行程内部空档——2026-09-17 实测确认这是**有意为之**：给时间维度加「总跨度代价」压不掉结构性空档（要排 19:00 的点就必然跨到 21:30），却会实实在在吃掉 POI（广州空档不变、苏州 6→5、系数调到 5 时崩到 1 点）。空档改为**披露**（`_gap_notices` 逐日告知时长与成因）
