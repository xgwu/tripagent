...
跨天片区分散规则：不同天安排不同片区——同一条马路/街区（如武康路、田子坊、外滩）及其 1 公里范围内的点位只能出现在其中一天，相邻两天绝不能去同一片区（用户要求贯穿性体验时除外，见贯穿偏好规则）；贯穿偏好规则：当用户表达的体验偏好按语义应贯穿全部天数（如「最好临湖」「每天都要湖景」「环湖骑行」），就每天围绕该偏好选不同的岸段/子片区（例：第一天太湖西山岛一线、第二天光福—湖东岸一线；或第一天金鸡湖环湖、第二天独墅湖/阳澄湖），同一天内仍集中相邻岸段顺路串联——此时跨天片区分散规则为用户偏好让位：不同天不得重复同一条道路/同一批点位，但允许同属一个大湖/大景区的不同岸段。若某天确实无法延续该偏好，必须在 reason 里写明原因。咖啡店等口碑配套每天最多 1 家，跟着当天主片区选，不要两天都往同一片区跑。配套顺路规则：咖啡店/餐厅等配套必须顺路——选位于当天相邻主选之间、或紧邻某个主选（步行可达，约 1.5 公里内）的店，绝不要为了某家网红店跨区绕路（例如上午在世博、下午去陆家嘴，就选两家之间沿线的店，不要折道武康路）。正餐规则：午餐/晚餐时段安排正经吃饭的地方（餐厅/名小吃/美食街），咖啡馆不能当正餐——咖啡只作为逛点之间的休憩加项，用户喜欢咖啡时另选顺路咖啡店，不要用它顶替午餐。
