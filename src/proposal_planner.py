...
招牌体验规则：先用世界知识判断用户需求的核心期待——每个城市都有公认必去的招牌景点（如上海的迪士尼度假区、北京环球影城、广州长隆），亲子/带娃类需求通常正期待这类招牌。若需求主题与某招牌景点高度匹配，必须把它作为主选排进某一天（独占一天，勿放 alternates）；只有当你有明确理由认为用户不会感兴趣（如需求明确排斥主题乐园）时才可不放。住宿锚点规则：用户指定住宿位置（如「住迪士尼附近」）时，行程必须包含该位置对应的标志性景点（住迪士尼附近则必含迪士尼），且该景点独占一天、优先安排在第一天，其余天数再安排其他区域。傍晚密度规则：博物馆/美术馆/展馆类场馆普遍 17 点前后闭馆，每天要为傍晚（17 点后）搭配至少 1 个晚间型停留点——夜展/灯光夜景/历史街区夜游/滨江步道/咖啡街区/书院茶馆等，避免傍晚大片空白；每天 4-6 个停留点中应含 1-2 个晚间型。慢节奏规则（针对老人/轮椅/行动不便/不要太累/慢节奏类需求，此规则下傍晚密度规则豁免）：每天 2-3 个停留点、绝不超过 4 个，以白天为主、尽量 17:30 前收尾，不安排夜间型点（除非用户明确提到夜景/夜市/夜游）；优先选择地势平缓、有无障碍条件、步行距离短的点位（平地园林主园区/滨湖步道/商业综合体/游船），避免登山型（虎丘山顶/山峰类）、石板路长距离古巷、需要大量站立排队的点位；同一天点位间车程尽量短，午后可安排 1 个茶馆/咖啡类慢休点，整体以从容、留有休息余量为准。
...
def plan(city: dict, query: str, days: int = 2, use_llm: bool = True,
    ...
    pkey = hashlib.sha1(json.dumps(
        [PROPOSE_PROMPT_VER, city["city"], days, norm_q, date0 or "",
         bool(weather_line), len(all_pois)],
